...
Step by step: the Docker install/enable/`usermod` lines are unchanged from before this revision. The new block right after it installs the `docker compose` CLI plugin by hand — Amazon Linux 2023's packaged `docker` doesn't ship it — by downloading a pinned release's Linux/x86_64 binary straight into Docker's CLI-plugins directory, controlled by `var.docker_compose_version` (default `v5.5.1`), then running `docker compose version` to fail loudly and immediately if the plugin didn't install correctly.

**This used to ask GitHub's API for the "latest" release tag at boot instead of using a pinned version** — that approach had two problems, one about reproducibility (unlike the pinned Terraform provider versions in Section 2, a boot today and a boot next year could install different compose binaries) and one that actually caused an outage: the API call had no retry and its output was never validated, so a transient network blip or a GitHub rate limit during boot left `docker-compose` missing/empty, `set -e` aborted the script right there, and `docker compose up -d` never ran — with no containers ever starting and no obvious error surfaced beyond `cloud-init-output.log`. The pinned download adds `curl --retry` for transient failures and an explicit `docker compose version` check so a broken install fails fast and visibly instead of silently.
...
| `docker_compose_version` | `v5.5.1` | No | `user_data`'s pinned `docker compose` CLI plugin download (Section 7) |
