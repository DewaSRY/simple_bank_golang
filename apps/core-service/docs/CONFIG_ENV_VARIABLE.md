...
| `DeviceFingerprintTrustedIPs` | `DEVICE_FINGERPRINT_TRUSTED_IPS` | `[]string` | *(blank — dev-only, e.g. `127.0.0.1`)* |
...
| `DEVICE_FINGERPRINT_TRUSTED_IPS` | `DeviceFingerprintTrustedIPs` | `[]string` (comma-separated) | *(blank)* | No — blank disables the bypass; **never set outside local dev**, it lets any request from a listed IP skip device-fingerprint binding entirely | [internal/api/auth/handler_utils.go](../internal/api/auth/handler_utils.go), [internal/api/router.go](../internal/api/router.go) |
