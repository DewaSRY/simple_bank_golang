...
func TestTransferTx(t *testing.T) {
	...
	fromAccount := sqlc.IncrementAccountBalanceRow{ID: arg.FromAccountID, Owner: "owner1", Balance: "900", Currency: "USD", CreatedAt: now}
	toAccount := sqlc.IncrementAccountBalanceRow{ID: arg.ToAccountID, Owner: "owner2", Balance: "1100", Currency: "USD", CreatedAt: now}
	...
	}{
		{
			...
			buildStubs: func(q *mockdb.MockQuerier) {
				gomock.InOrder(
					...
					q.EXPECT().IncrementAccountBalance(gomock.Any(), sqlc.IncrementAccountBalanceParams{
			...
			buildStubs: func(q *mockdb.MockQuerier) {
				gomock.InOrder(
					...
					q.EXPECT().IncrementAccountBalance(gomock.Any(), sqlc.IncrementAccountBalanceParams{
						ID:      arg.FromAccountID,
						Balance: "-100",
					}).Return(sqlc.IncrementAccountBalanceRow{}, errors.New("update from balance error")),
				)
			},
			checkResponse: func(t *testing.T, result TransferTxResult, err error) {
			...
			buildStubs: func(q *mockdb.MockQuerier) {
				gomock.InOrder(
					...
					q.EXPECT().IncrementAccountBalance(gomock.Any(), sqlc.IncrementAccountBalanceParams{
						ID:      arg.ToAccountID,
						Balance: arg.Amount,
					}).Return(sqlc.IncrementAccountBalanceRow{}, errors.New("update to balance error")),
				)
			},
			checkResponse: func(t *testing.T, result TransferTxResult, err error) {
